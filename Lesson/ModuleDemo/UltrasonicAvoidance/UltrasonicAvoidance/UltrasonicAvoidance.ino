...
void loop() {
	...
	if ((da >= 50) && (da <= 2000) && (left > 0) && (right > 0)) {
		int val = 150;
	...
	if ((left == 0) && (right > 0) && (da >= 30 && da <= 1000)) {
		analogWrite(INPUT1_PIN, 200); //the speed value of motorA is val
		analogWrite(INPUT2_PIN, 0);
		analogWrite(INPUT3_PIN, 0);
		analogWrite(INPUT4_PIN, 130); //the speed value of motorA is val
	...
	if ((left > 0) && (right == 0) && (da >= 30 && da <= 1000)) {
		analogWrite(INPUT1_PIN, 130);//the speed value of motorA is 200
		analogWrite(INPUT2_PIN, 0); 
		analogWrite(INPUT3_PIN, 0);
